...
<span style='color:red; font-size:18px'><b>T</b></span>: Terno
    ...
    colpi = st.slider("Numero di estrazioni da analizzare", 50, 300, 100, step=10)
    ...
    frequenze, ritardi, affinità = {}, {}, {n: 0 for n in numeri_finali}
    ...
    for _, row in df_filtrato.iterrows():
        estratti_turno = [int(row[c]) for c in cols if pd.notnull(row[c])]
        presenti = [n for n in numeri_finali if n in estratti_turno]
        for n in presenti:
            frequenze[n] += 1
    ...
    df_risultati = pd.DataFrame(risultati).sort_values("Punteggio", ascending=False).reset_index(drop=True)
