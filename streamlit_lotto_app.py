...
st.title("💰 Simulazione Gioco - Ambo & Terno con Recupero")
...
if uploaded_file:
    ...
    for data in tutte_date[:-5]:  # Escludiamo le ultime 5 per sicurezza
        row = df[df["Data"] == data]
        if row.empty:
            continue
    ...
    st.markdown("### 📊 Risultati della simulazione (1 blocco ogni 5 estrazioni)")
