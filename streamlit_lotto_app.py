...
if uploaded_file:
    ...
    if st.button("🎲 Calcola numeri da giocare"):
        ...
        if not numeri_base:
            ...
            # Evidenzia i numeri consigliati in base alla loro categoria e se sono usciti
            ...
            for num in migliori:
                if num in numeri_base and num in tutti_successivi:
                    ...
                    consigliati_colorati.append(str(num))
            st.markdown(", ".join(consigliati_colorati), unsafe_allow_html=True)

            # Titolo + estratti base sotto
            st.markdown("### 🔁 Ripetizioni nei 5 turni successivi")
            st.markdown(f"<p style='font-size:14px; margin-top:-10px;'>Estratti base: <code>{', '.join(map(str, diretti))}</code></p>", unsafe_allow_html=True)
                ...
                for riga in righe:
                    ...
                    for num in riga["Numeri usciti"]:
                        if num in diretti:
                            ...
                            colorati.append(str(num))
                    line = f"🗓️ {riga['Data']} → Usciti: " + ", ".join(colorati)
                    st.markdown(line, unsafe_allow_html=True)

            with col_legenda:
                st.markdown("**Legenda**")
                st.markdown("<span style='color:green'><b>Verde</b></span>: Estratti base<br><span style='color:orange'><b>Arancione</b></span>: Precedenti/Successivi", unsafe_allow_html=True)
