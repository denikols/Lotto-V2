...
if uploaded_file:
    ...
    # Ottieni il valore selezionato usando la key
    ruota_input = st.selectbox("Scegli la ruota per calcolo base", ruote, key="ruota_input")
    # Memorizza il valore (che sarà una stringa)
    st.session_state['ruota_input_value'] = ruota_input

    # Fai lo stesso per gli altri widget
    data_estrazione = st.date_input("Data estrazione", value=df["Data"].max().date(), key="data_estrazione")
    st.session_state['data_estrazione_value'] = data_estrazione
    ...
    st.session_state['numero_spia_input_value'] = numero_spia_input
    ...
    st.session_state['finestra_analisi_spia_value'] = finestra_analisi_spia
