...
if uploaded_file:
    ...
    # 🔍 Controllo delle 5 estrazioni successive
    ...
    if prossime.empty:
        ...
        for _, row in prossime.iterrows():
            ...
            match_str = ', '.join(str(n) if n not in match else f"**{n}**" for n in estratti)
            st.markdown(f"🗓️ {row['Data'].date()} → {match_str} {simbolo}")
