...
if uploaded_file:
    ...
    with results_container:
        ...
        <span style='color:red; font-size:18px'><b>T</b></span>: Terno
                """, unsafe_allow_html=True)
                # Nuova sezione per l'analisi completa dei 15 numeri
    st.markdown("### 🔍 Analisi Approfondita dei 15 Numeri")
    
    # Organizziamo le analisi in tabs per maggiore chiarezza
    ...
    def analizza_forza_pattern(df, ruota, numeri_15, data_riferimento):
        df_analisi = df[df['Data'] <= data_riferimento].sort_values('Data', ascending=False).head(100)
        risultati = {
            'forza': {},
            'posizioni': {},
            'frequenza_ultimo_mese': {},
            'ambi_formati': Counter(),
            'terni_formati': Counter()
        }
        
        for num in numeri_15:
            risultati['forza'][num] = 0
            risultati['posizioni'][num] = Counter()
            risultati['frequenza_ultimo_mese'][num] = 0

        for _, row in df_analisi.iterrows():
            estratti = []
            for i in range(5):
                colonna = f"{ruota}{'.'+str(i) if i > 0 else ''}"
                if pd.notnull(row[colonna]):
                    num = int(row[colonna])
                    if num in numeri_15:
                        estratti.append(num)
                        risultati['forza'][num] += 1
                        risultati['posizioni'][num][i+1] += 1
                        
                        # Controlla se è nell'ultimo mese
                        if (data_riferimento - row['Data']).days <= 30:
                            risultati['frequenza_ultimo_mese'][num] += 1

            # Analisi ambi e terni
            for i, n1 in enumerate(estratti):
                for j, n2 in enumerate(estratti[i+1:], i+1):
                    risultati['ambi_formati'][(min(n1,n2), max(n1,n2))] += 1
                    for n3 in estratti[j+1:]:
                        risultati['terni_formati'][(min(n1,n2,n3), 
                                                  sorted([n1,n2,n3])[1],
                                                  max(n1,n2,n3))] += 1
        
        return risultati
    def analizza_combinazioni(df, ruota, numeri_base, numeri_ant_succ, data_riferimento):
        df_analisi = df[df['Data'] <= data_riferimento].sort_values('Data', ascending=False).head(50)
        risultati = {
            'base_ant': Counter(),  # Combinazioni base-antecedenti
            'base_succ': Counter(), # Combinazioni base-successivi
            'ant_succ': Counter(),  # Combinazioni antecedenti-successivi
            'sequenze': []          # Sequenze di uscita
        }
        ...
        for _, row in df_analisi.iterrows():
            ...
            for i in range(5):
                ...
                if pd.notnull(row[colonna]):
                    num = int(row[colonna])
                    if num in numeri_base + numeri_ant_succ:
                        estratti.append(num)
            
            # Analisi delle combinazioni
            for n1 in estratti:
                for n2 in estratti:
                    if n1 in numeri_base and n2 in numeri_ant_succ:
                        risultati['base_ant'][(min(n1,n2), max(n1,n2))] += 1
                    if n1 in numeri_base and n2 in numeri_ant_succ:
                        risultati['base_succ'][(min(n1,n2), max(n1,n2))] += 1
                    if n1 in numeri_ant_succ and n2 in numeri_ant_succ:
                        risultati['ant_succ'][(min(n1,n2), max(n1,n2))] += 1
            
            if len(estratti) >= 2:
                risultati['sequenze'].append(estratti)
        
        return risultati
    def analizza_ciclicita(df, ruota, numeri_15, data_riferimento):
        df_analisi = df[df['Data'] <= data_riferimento].copy()
        df_analisi['GiornoSettimana'] = df_analisi['Data'].dt.day_name()
        df_analisi['Mese'] = df_analisi['Data'].dt.month_name()
        ...
        for num in numeri_15:
            date_uscite = []
            for _, row in df_analisi.iterrows():
                estratti = [int(row[f"{ruota}{'.'+str(i) if i > 0 else ''}"]) 
                          for i in range(5) 
                          if pd.notnull(row[f"{ruota}{'.'+str(i) if i > 0 else ''}"])]  # Corretta questa riga
                if num in estratti:
                    date_uscite.append(row['Data'])
                    risultati['giorni'][row['GiornoSettimana']] += 1
                    risultati['mesi'][row['Mese']] += 1
            
            # Calcola intervalli tra le uscite
            if len(date_uscite) >= 2:
                intervalli = [(date_uscite[i] - date_uscite[i+1]).days 
                            for i in range(len(date_uscite)-1)]
                risultati['cicli'][num] = {
                    'media_intervallo': sum(intervalli) / len(intervalli),
                    'intervallo_min': min(intervalli),
                    'intervallo_max': max(intervalli)
                }
        ...
        return risultati
    def analizza_statistiche_avanzate(df, ruota, numeri_15, data_riferimento):
        risultati = {
            'distribuzione_decine': Counter(),
            'pari_dispari': {'pari': 0, 'dispari': 0},
            'figure_geometriche': [],
            'somma_media': [],
            'distanze': Counter()
        }
        ...
        for num in numeri_15:
            # Decine
            risultati['distribuzione_decine'][(num-1)//10] += 1
            # Pari/Dispari
            if num % 2 == 0:
                risultati['pari_dispari']['pari'] += 1
            else:
                risultati['pari_dispari']['dispari'] += 1
        
        # Analisi delle figure geometriche
        # Triangoli
        for i, n1 in enumerate(numeri_15):
            for j, n2 in enumerate(numeri_15[i+1:], i+1):
                for n3 in numeri_15[j+1:]:
                    if abs(n2-n1) == abs(n3-n2):
                        risultati['figure_geometriche'].append(
                            ('triangolo', (n1,n2,n3))
                        )
        
        # Analisi delle ultime 10 estrazioni
        df_recenti = df[df['Data'] <= data_riferimento].sort_values('Data', ascending=False).head(10)
        for _, row in df_recenti.iterrows():
            estratti = [int(row[f"{ruota}{'.'+str(i) if i > 0 else ''}"]) 
                       for i in range(5) 
                       if pd.notnull(row[f"{ruota}{'.'+str(i) if i > 0 else ''})"])]
            numeri_presenti = [n for n in estratti if n in numeri_15]
            if numeri_presenti:
                risultati['somma_media'].append(sum(numeri_presenti)/len(numeri_presenti))
                
                # Calcola distanze tra numeri consecutivi
                numeri_ordinati = sorted(numeri_presenti)
                for i in range(len(numeri_ordinati)-1):
                    risultati['distanze'][numeri_ordinati[i+1] - numeri_ordinati[i]] += 1
        
        return risultati

    # Esegui le analisi
    numeri_15 = sorted(set(diretti + successivi))
    analisi_forza = analizza_forza_pattern(df, ruota_input, numeri_15, pd.Timestamp(data_estrazione))
    analisi_combinazioni = analizza_combinazioni(df, ruota_input, diretti, successivi, pd.Timestamp(data_estrazione))
    analisi_ciclicita = analizza_ciclicita(df, ruota_input, numeri_15, pd.Timestamp(data_estrazione))
    analisi_statistiche = analizza_statistiche_avanzate(df, ruota_input, numeri_15, pd.Timestamp(data_estrazione))

    # Visualizza i risultati nelle tabs
    with tab1:
        st.markdown("#### 💪 Forza dei Numeri")
        
        # Crea un DataFrame per la visualizzazione
        df_forza = pd.DataFrame([
            {
                'Numero': num,
                'Forza': analisi_forza['forza'][num],
                'Freq. Ultimo Mese': analisi_forza['frequenza_ultimo_mese'][num],
                'Posizione Preferita': max(analisi_forza['posizioni'][num].items(), 
                                         key=lambda x: x[1])[0] if analisi_forza['posizioni'][num] else '-'
            }
            for num in numeri_15
        ]).sort_values('Forza', ascending=False)
        
        st.dataframe(df_forza)
        
        st.markdown("#### 🎯 Ambi più Frequenti")
        for (n1, n2), freq in analisi_forza['ambi_formati'].most_common(5):
            st.write(f"{n1}-{n2}: {freq} volte")

    with tab2:
        st.markdown("#### 🔄 Pattern di Combinazione")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("**Base con Antecedenti**")
            for (n1, n2), freq in analisi_combinazioni['base_ant'].most_common(5):
                st.write(f"{n1}-{n2}: {freq} volte")
        
        with col2:
            st.markdown("**Base con Successivi**")
            for (n1, n2), freq in analisi_combinazioni['base_succ'].most_common(5):
                st.write(f"{n1}-{n2}: {freq} volte")

    with tab3:
        st.markdown("#### 📅 Analisi Temporale")
        
        # Visualizza giorni preferiti
        giorni_ord = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 
                     'Friday', 'Saturday', 'Sunday']
        giorni_ita = ['Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 
                     'Venerdì', 'Sabato', 'Domenica']
        
        df_giorni = pd.DataFrame([
            {'Giorno': g_ita, 'Frequenza': analisi_ciclicita['giorni'][g_eng]}
            for g_eng, g_ita in zip(giorni_ord, giorni_ita)
        ])
        
        st.bar_chart(df_giorni.set_index('Giorno'))
        
        # Mostra cicli medi per numero
        st.markdown("#### 🔄 Cicli di Uscita")
        for num, ciclo in analisi_ciclicita['cicli'].items():
            st.write(f"Numero {num}:")
            st.write(f"- Media giorni tra le uscite: {ciclo['media_intervallo']:.1f}")
            st.write(f"- Intervallo min-max: {ciclo['intervallo_min']}-{ciclo['intervallo_max']} giorni")
    ...
    with tab4:
        ...
        with col1:
            ...
            for decina, count in sorted(analisi_statistiche['distribuzione_decine'].items()):
                st.write(f"Decina {decina+1}: {count} numeri")
        
        with col2:
            st.markdown("**Pari/Dispari**")
            st.write(f"Pari: {analisi_statistiche['pari_dispari']['pari']}")
            st.write(f"Dispari: {analisi_statistiche['pari_dispari']['dispari']}")
        
        if analisi_statistiche['figure_geometriche']:
            st.markdown("#### 🔷 Figure Geometriche")
            for tipo, numeri in analisi_statistiche['figure_geometriche']:
                st.write(f"{tipo.title()}: {'-'.join(map(str, numeri))}")
        
        if analisi_statistiche['somma_media']:
            st.markdown("#### 📈 Statistiche Numeriche")
            media_totale = sum(analisi_statistiche['somma_media']) / len(analisi_statistiche['somma_media'])
            st.write(f"Media dei numeri quando escono insieme: {media_totale:.1f}")
