...
if uploaded_file:
    ...
    # ✅ Messaggio visivo per segnalare se la data contiene un'estrazione
    if pd.Timestamp(data_estrazione) in df["Data"].values:
        st.markdown("<span style='color:green'><b>✅ Estrazione disponibile in questa data</b></span>", unsafe_allow_html=True)
    else:
        st.markdown("<span style='color:red'><b>❌ Nessuna estrazione trovata in questa data</b></span>", unsafe_allow_html=True)
