...
if uploaded_file:
    ...
    def stile_riga(row):
        num = row["Numero"]
        base_style = "text-align: center; font-size: 22px;"
        if num in estratti:
            return [base_style + "color: red; font-weight: bold; text-align: center;"] * len(row)
        elif num in successivi:
            return [base_style + "color: green; font-weight: bold; text-align: center;"] * len(row)
        else:
            return [base_style + "text-align: center;"] * len(row)

    st.markdown("### 📊 Frequenza storica evidenziata")
    styled_df = df_freq.style.apply(stile_riga, axis=1)
