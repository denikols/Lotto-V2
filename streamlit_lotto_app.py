...
if uploaded_file:
    ...
    st.markdown(
        ...
        " ".join(f"<span style='display:inline-block; width:14px; height:14px; background:red; border-radius:50%; margin-right:6px'></span>{d.strftime('%Y-%m-%d')}" for d in sorted(df['Data'].dropna().dt.date.unique())) +
        "</div>",
