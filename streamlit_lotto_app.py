        ...
        with st.spinner("Analisi in corso..."):
            try:
                ...
                if risultati_spia:
                    ...
                    st.markdown(f"<p style='font-size:16px; color:green;'>Il numero spia <b>{numero_spia_input}</b> è uscito <b>{totale_occorrenze}</b> volte nelle ultime {finestra_analisi_spia} estrazioni sulla ruota di <b>{ruota_input}</b>.</p>", unsafe_allow_html=True)
                    st.markdown(f"<p style='font-size:16px; color:green;'>I numeri con maggiore probabilità di uscire nei prossimi turni sono: <br> {', '.join(numeri_suggeriti)}</p>", unsafe_allow_html=True)
                    
                    # Visualizzazione tabella dei risultati
                    df_risultati = pd.DataFrame(risultati_spia, columns=["Numero", "Probabilità"])
                    df_risultati["Probabilità"] = df_risultati["Probabilità"].apply(lambda x: f"{x:.2%}")
                    st.dataframe(df_risultati)
                else:
                    st.info(f"L'analisi del numero spia {numero_spia_input} sulla ruota di {ruota_input} non ha prodotto suggerimenti significativi. Il numero non è apparso nelle ultime {finestra_analisi_spia} estrazioni.")
