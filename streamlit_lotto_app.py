...
if uploaded_file:
    ...
    if estrazione.empty:
        ...
        # Calcola l'altezza necessaria dinamicamente
        numero_righe = len(df_tabella)
        altezza = (numero_righe + 1) * 35 + 3

        col1, col2 = st.columns([1.5, 2.5])
        with col1:
            st.markdown("### 🎯 Numeri estratti")
            st.dataframe(df_tabella.transpose(), use_container_width=False, height=altezza, width=600)
