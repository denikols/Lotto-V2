...
/* Stile per il pulsante di stampa personalizzato */
.custom-print-button {
...
# Funzione per generare PDF download (alternativa alla stampa)
def create_download_link(content, filename):
    b64 = base64.b64encode(content.encode()).decode()
    return f'<a download="{filename}" href="data:application/octet-stream;base64,{b64}" class="custom-print-button">📥 Scarica Report</a>'

# Soluzione più robusta: Utilizziamo un iframe nascosto per attivare la stampa
st.markdown("""
<div style="display: flex; justify-content: flex-end; margin-bottom: 10px;">
    <button id="printButton" class="custom-print-button" onclick="printContent()">🖨️ Stampa analisi</button>
</div>
...
function printContent() {
    // Cattura il contenuto da stampare
    var printContent = document.querySelector('.stApp').innerHTML;
    
    // Stili da applicare per la stampa
    var printStyles = `
        <style>
            body { font-family: Arial, sans-serif; }
            .stButton, .stDownloadButton, .custom-print-button, 
            [data-testid="stFileUploadDropzone"], .stToolbar { display: none !important; }
    ...
    // Prepara il documento nell'iframe
    var printFrame = document.getElementById('printFrame');
    var printWindow = printFrame.contentWindow;
    
    // Scrive il contenuto nell'iframe
    printWindow.document.open();
    printWindow.document.write('<html><head>'+printStyles+'</head><body>');
    printWindow.document.write('<div class="print-container">');
    printWindow.document.write('<h1>Report Analisi Lotto</h1>');
    printWindow.document.write('<p>Data generazione: '+new Date().toLocaleString()+'</p><hr>');
...
if uploaded_file:
    ...
    # Crea contenuto per download HTML come alternativa alla stampa
    html_content = f"""
    <html>
    <head>
    ...
    # Aggiungi link per scaricare il report HTML
    st.markdown(create_download_link(html_content, f"report_lotto_{pd.Timestamp.now().strftime('%Y%m%d')}.html"), unsafe_allow_html=True)
    
    # Mostra un messaggio informativo sulla stampa
    st.info("💡 Se il pulsante di stampa non funziona, puoi provare questi metodi alternativi:\n\n"
            "1. Utilizzare la combinazione di tasti CTRL+P (o ⌘+P su Mac)\n"
            "2. Scaricare il report HTML usando il pulsante verde in alto a destra\n"
            "3. Fare clic destro sulla pagina e selezionare 'Stampa'")
